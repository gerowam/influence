...
#define outlog(format, args...) fprintf(stderr, format, args); 
#define plainlog(string) fprintf(stderr, string);
#define plainbug(string) if (FLAGS_debug) fprintf(stderr, string);

#ifdef GDEBUG
#define debug(format, args...) if (FLAGS_debug) fprintf(stderr, format, args);
#else
#define debug(format, args...)// if (FLAGS_debug) fprintf(stderr, format, args);
#endif
...
static inline double vget(const gsl_vector* v, int i) { return(gsl_vector_get(v, i)); };
static inline void vset(gsl_vector* v, int i, double x) { gsl_vector_set(v, i, x); };

// Increment a vector element by a double.
void vinc(gsl_vector*, int, double);

static inline double mget(const gsl_matrix* m, int i, int j) { return(gsl_matrix_get(m, i, j)); };
static inline void mset(gsl_matrix* m, int i, int j, double x) { gsl_matrix_set(m, i, j, x); };
...
void optimize(int dim, gsl_vector* x, void* params, void (*fdf)(const gsl_vector*, void*, double*, gsl_vector*),
