...
DEFINE_int64(rng_seed, 0, "Specifies the random seed.  If 0, seeds pseudo-randomly.");
...
void vct_fscanf(const char* filename, gsl_vector* v) {
    outlog("reading %ld vector from %s\n", v->size, filename);
    FILE* fileptr;

    if (!fileptr) {
      outlog("Error opening file %s. Failing.\n", filename);
      exit(1);
    }
...
void choose_k_from_n(int k, int n, int* result) {
	...
	if (FLAGS_rng_seed) 
	  gsl_rng_set(RANDOM_NUMBER_GENERATOR, FLAGS_rng_seed); 
    }

    for (i = 0; i < n; i++)
        x[i] = i;

    gsl_ran_choose (RANDOM_NUMBER_GENERATOR, (void *) result,  k, (void *) x, n, sizeof(int));
}
 ...
 *
 */
void optimize_fdf(int dim, gsl_vector* x, void* params, void (*fdf)(const gsl_vector*, void*, double*, gsl_vector*),
                  void (*df)(const gsl_vector*, void*, gsl_vector*), double (*f)(const gsl_vector*, void*),
                  double* f_val, double* conv_val, int* niter) {
    ...
    gsl_multimin_fdfminimizer * opt = gsl_multimin_fdfminimizer_alloc(method, dim);
 ...
 *
 */
void optimize_f(int dim, gsl_vector* x, void* params, double (*f)(const gsl_vector*, void*)) {
    gsl_multimin_function obj;
    obj.f = f;
    obj.n = dim;
    obj.params = params;

    const gsl_multimin_fminimizer_type * method = gsl_multimin_fminimizer_nmsimplex;
...
// Added by AG to get different, but deterministically seeded 
// random multivariate Gaussians with which to initialize mu
gsl_rng* new_random_number_generator(const long seed) {
    gsl_rng* random_number_generator = gsl_rng_alloc(gsl_rng_taus);

    long t1 = (FLAGS_rng_seed * seed);
    (void) time(&t1);

    printf("SPECIFIED RANDOM SEED = %ld\n", t1);
    gsl_rng_set(random_number_generator, t1);
