...
char *dequote (char *s) {
    ...
    if (s[0] != '\'') return s;
       ...
       error((char* const) "Illegal string passed to dequote: %s", s);
    if (sbuf == NULL)
...
void * util_malloc (int size)
{
    ...
    if (p == NULL) error((char* const) "UTIL_MALLOC: Ran out of space. Space in use: %d (%d pointers)\n",
...
void * util_calloc (int num, int size)
{
    ...
    if (p == NULL) error((char* const) "UTIL_CALLOC: Ran out of space. Space in use: %d (%d pointers)\n",
                                           space_in_use, pointers_in_use);
...
void * util_realloc (void * p, int size)
{
    ...
    if (realp == NULL) error((char* const) "UTIL_REALLOC: Ran out of space. Space in use: %d (%d pointers)\n",
                                           space_in_use, pointers_in_use);
    *((int *)(realp)) = size;
