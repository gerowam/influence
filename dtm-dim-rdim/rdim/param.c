...
static  char *begin_restart_header=(char* const) "/***BEGIN_RESTART***/\n";
static  char *end_restart_header=(char* const) "/***END_RESTART***/\n";
static  int  NumberOfYesStrings = 3;
static  char *YesString[] = {(char* const) "YES", (char* const) "TRUE", (char* const) "1"};
...
int param_init(){
   ...
   ddinfName = getenv((char* const) "PARAM");
   ddinf_inited = TRUE;
...
//   printf("ddinf: %d\n", ddinf); // Removed by AG
   ...
   while (ddinf != NULL) {
	  ...
	  case '/':
            if ((state == QUOTATION)  || (state == DQUOTATION))
	          ...
	          if (ungetc(next_c, ddinf->stream) == EOF)
                      error((char* const) "PARAM: ungetc failed.");
	       }
	    break;
	  case '*':
            if ((state == QUOTATION) || (state == DQUOTATION))
	 ...
	 switch (state) {
	    ...
	    switch (cclass) {
	     ...
	     case COMMENT_START:
	       ...
	       state = COMMENT;
               commentLevel++;
	       break;
	     case COMMENT_END: ddinf_error((char* const) "Unmatched comment ending");
	     case DELIMITER: ddinf_error((char* const) "Missing parameter name.");
	     case QUOTE: ddinf_error((char* const) "Quotations for values only.");
	     case DQUOTE: ddinf_error((char* const) "Quotations for values only.");
	  ...
	  case ENDMARKED:
	    switch (cclass) {
	     ...
	     case QUOTE: ddinf_error((char* const) "Quotations for values only.");
	     case DQUOTE: ddinf_error((char* const) "Quotations for values only.");
	     case ENDMARK: ddinf_error((char* const) "Null production");
	     case COMMAND: commandBufferIndex = 0; state = DIRECTIVE; break;
	     case OTHER:
	       ddname_buffer[nameBufferIndex++] = c;
	       state = NAME;
	     ...
	     case END_OF_LINE: state = BEGIN_LINE; break;
 	     default: 
	       ddinf_error((char* const) "Illegal character after end marker.");
	    }
	    break;
	  case COMMENT:
            switch (cclass) {
	  ...
	  case NAME:
	    switch (cclass) {
	     ...
	     case COMMENT_START:
	       ...
	       break;
	     case COMMENT_END: ddinf_error((char* const) "Unmatched comment ending");
	     case QUOTE: ddinf_error((char* const) "Quotes for values only.");
	     case DQUOTE: ddinf_error((char* const) "Quotes for values only.");
	     case DELIMITER:
	       state = VALUE;
	       break;
	     case ENDMARK: ddinf_error((char* const) "Missing value.");
	     case COMMAND:  /* XXX shouldn't this be allowed if name is begun ?*/
	       ddinf_error((char* const) "Illegal use of COMMAND character.");
             case FIXIT:
	       ddinf_error((char* const) "Illegal use of FIXIT character.");
	  ...
	  case NAME_ENDED:
	    switch (cclass) {
	     ...
	     case DELIMITER:
	       ...
	       break;
	     default:
	       ddinf_error((char* const) "Illegal character after parameter name.");
	    }
	    break;
	  case VALUE:
	    switch (cclass) {
	     ...
	     case COMMENT_START:  /* XXX be sure this means not quoted!! */
	        ...
	        break;
	     case COMMENT_END: ddinf_error((char* const) "Unmatched comment ending");
	     case DELIMITER: ddinf_error((char* const) "Missing delimiter or endmark.");
	     case ENDMARK:
                ddname_buffer[nameBufferIndex++] = EOS;
                ddvalue_buffer[valueBufferIndex++] = EOS;
                if (insert(ddname_buffer, strip(ddvalue_buffer)) == NULL)
                   ddinf_error((char* const) "error adding (name,value) pair");
                nameBufferIndex = 0;
                valueBufferIndex = 0;
	        state = ENDMARKED;
	        break;
	     /* case END_OF_LINE: ddinf_error((char* const) "Missing end marker."); */
	  ...
	  case DIRECTIVE:
	    switch (cclass) {
	     ...
	     default: ddinf_error((char* const) "Illegal character in ddinf directive");
	    }
	    break;
	  case ARGUMENT:
	    switch (cclass) {
	     ...
	     case ENDMARK:
		  ...
		  if (ddinf->stream == NULL)
		    ddinf_error((char* const) "Unable to open ddinf");
		  state = BEGIN_LINE;
	       }
		  ...
		  state = BEGIN_LINE;
	       ...
	       else ddinf_error((char* const) "Unrecognized ddinf directive.");
	       break;
	     case END_OF_LINE: ddinf_error((char* const) "Missing end marker");
             default:
	  ...
	  case QUOTATION_ENDED:
	    switch (cclass) {
		 ...
		 state = COMMENT;
                 commentLevel++;
		 break;
	       case COMMENT_END: ddinf_error((char* const) "Unmatched comment end.");
	       case ENDMARK:
                 ddname_buffer[nameBufferIndex++] = EOS;
                 ddvalue_buffer[valueBufferIndex++] = EOS;
		 ...
		 break;
	       default:
		 ddinf_error((char* const) "Illegal character after quotation.");
	    }
            break;
	 ...
	 if ((state == QUOTATION) || (state == DQUOTATION)) {
            ddinf_error((char* const) "end of file inside quoted string.");
	 }
         else if (commentLevel != 0) {
            ddinf_error((char* const) "end of file inside comment.");
         }
	 else {
            ddinf_error((char* const) "Not in proper state at end of file.");
         }
      }
      pop_ddinf();
...
char *lookup_ddval(const char *p, int *len)
{
    ...
    if (p[0] !='(')
	  ...
	  *len = strlen(p)+1;
	  ...
	  if (nopenparens>0) error((char* const) "PARAM: Unmatched (");
    ...
    if (strchr(ddparm, EMBEDDED_VAR_CHAR)!=NULL)
      error((char* const) "PARAM: No nested %% allowed.");

    if ((lp = lookup(ddparm)) == NULL)
      error((char* const) "PARAM: symbol %s not defined (yet)\n", ddparm);
    return lp->ddvalue;
}
...
ddlink *insert (const char *newddname, const char *newddvalue_raw) {
    ...
    if ((lp = lookup(newddname)) == NULL) {
       ...
       if (strcmp(lp->ddvalue, newddvalue) == 0)
          printf((char* const) "PARAM: Warning: Symbol %s fixed more than once\n", lp->ddname);
       else
          error((char* const) "PARAM: Error: Symbol %s already fixed", lp->ddname);
    }
    return lp;
}
...
int param_checkpointed(void){
   ...
   if (restart_is_new || qfilef(param_getc((char* const) "%restart",(char* const) ""))==0)
      return FALSE;
   else {
      param_init_restart();
      checkpointed = TRUE;
...
ddinf_link *push_last_complete_block (char *inname) {
   ...
   if (!(unit = fopen(inname, "r")))
      error((char* const) "PARAM: Unable to open restart %s", inname);
...
void param_init_restart(void){
   ...
   if (restart_name==NULL)
     {
       restart_name = (char *) malloc(strlen(param_getc((char* const) "%restart",(char* const) ""))+1);
       strcpy(restart_name, param_getc((char* const) "%restart",(char* const) ""));
     }

   if (restart_is_new || qfilef(restart_name)==0) {
      if (!(restart_unit = fopen(restart_name, "w")))
         error((char* const) "PARAM: Unable to open restart %s", restart_name);
      fputs(begin_restart_header, restart_unit);
      fflush(restart_unit);
   }
   else {
...
ddinf_link *push_ddinf (char *inname) {
    ...
    if ((dl->stream = fopen(dl->name, "r")) == NULL)
       error((char* const) "PARAM: Unable to open ddinf %s", dl->name);
    return dl;
}
...
char *param_gets(const char *var_name){
    ...
    param_init_par();
    if ((lp = prefixed_lookup(var_name)) == NULL) {
	 error((char* const) "PARAM: variable %s not found.", var_name);
	 return NULL;
    }
    else return lp->ddvalue;
    }
...
/*******************************************************/
void  ddinf_error (char *ermsg) {
    error((char* const) "ERROR IN DDINF=%s: %s", ddinf->name, ermsg);
}
...
long safe_atol (const char *name, const char *expr) {
    ...
    if (strcmp(suffix,"") != 0)
       error((char* const) "PARAM: Unable to convert value \"%s\" for ddname \"%s\"",
       expr, name);
    return d;
}
