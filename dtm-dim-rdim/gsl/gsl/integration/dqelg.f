      ...
      do 10 i=0,999
         ...
         print 100,n,x,result,abserr,nres,res3la(1),res3la(2),res3la(3)
         do 20 j=1,10
            print *,'epstab[',j,'] = ',epstab(j)
 20      continue
         x = x + d
         d = d / c
         c = c * 1.01
 10   continue
 100  format('n=',i4,' x=',f20.15,' result=',f20.15,
     $     ' abserr=',e15.10,' nres=',i4,' res3la=',
     $     f8.3,f8.3,f8.3)
      end
...
C***FIRST EXECUTABLE STATEMENT  DQELG
      EPMACH = 2.2204460492503131e-16
      OFLOW = 1.7976931348623157e+308
      NRES = NRES+1
      ABSERR = OFLOW
      RESULT = EPSTAB(N)
      IF(N.LT.3) GO TO 100
...
C           OF N.
C
        IF(EPSINF.LE.0.1D-03) print *,'EPSINF.LE.0.1D-03'
...
C
   50 IF(N.EQ.LIMEXP) N = 2*(LIMEXP/2)-1
      ...
      DO 60 I=1,IE
        IB2 = IB+2
         print *,'A:copying epstab(',ib2,') into epstab(',ib,')'
   ...
   60 CONTINUE
      ...
      INDX = NUM-N+1
      DO 70 I = 1,N
         print *,'B:copying epstab(',indx,') into epstab(',i,')'
...
C
   90 ABSERR = ABS(RESULT-RES3LA(3))+ABS(RESULT-RES3LA(2))
     1  +ABS(RESULT-RES3LA(1))
      print 61, abserr
 61   format("error estimate computed as ",e10.5)
      print 62, result
      print 62, RES3LA(3)
      print 62, RES3LA(2)
      print 62, RES3LA(1)
 62   format("term = ",e15.7)
      RES3LA(1) = RES3LA(2)
      RES3LA(2) = RES3LA(3)
      RES3LA(3) = RESULT
