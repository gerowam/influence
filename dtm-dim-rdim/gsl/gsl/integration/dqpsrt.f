      program main
      integer limit, last, maxerr, nrmax
      double precision ermax
      double precision elist
      dimension elist(30), iord(30)
...
C ***JUMP OUT OF DO-LOOP
        ...
        IORD(NRMAX) = ISUCC
        print *,"NR copied iord(",nrmax-1,") to iord(",nrmax,")"
        NRMAX = NRMAX-1
   20    CONTINUE
C
...
C ***JUMP OUT OF DO-LOOP
        ...
        IORD(I-1) = ISUCC
        print *, "DOWN copied iord(", i, ") to iord(",i-1,")" 
   40 CONTINUE
   50 IORD(JBND) = MAXERR
      IORD(JUPBN) = LAST
      GO TO 90
...
C
   60 IORD(I-1) = MAXERR
      print *, "I-1 put ", MAXERR, " into iord(",I-1,")"
      K = JBND
      DO 70 J=I,JBND
        ISUCC = IORD(K)
C ***JUMP OUT OF DO-LOOP
        ...
        IORD(K+1) = ISUCC
        print *, "UP copied iord(", k, ") to iord(",K+1,")" 
        K = K-1
   70 CONTINUE
      IORD(I) = LAST
      print *, "I put ", last, " into iord(",I,")"
      GO TO 90
   80 IORD(K+1) = LAST
      print *, "K put ", last, " into iord(",K+1,")"
C
